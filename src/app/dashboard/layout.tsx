...
export default function DashboardLayout({
    ...
    return (
        <div className="flex h-screen bg-zinc-950 text-zinc-100 selection:bg-indigo-500/30">
            {/* Sidebar */}
            {/* Sidebar */}
            <aside className="w-72 border-r border-zinc-800 bg-zinc-950/95 flex flex-col backdrop-blur-sm">
                <div className="p-8 pb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xl font-bold tracking-tight text-zinc-100">IMPCTFUL</span>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 space-y-8">
                    ...
                    <nav className="space-y-1.5">
                        ...
                        <Link
                            href="/onboarding"
                            className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200 transition-all duration-200 group"
                        >
                            <Compass className="h-4 w-4 text-zinc-500 group-hover:text-zinc-400" />
                            Onboarding
                        </Link>

                        <button className="flex w-full items-center rounded-lg px-3 py-2.5 text-sm font-medium text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200 transition-all duration-200 group">
                    ...
                    <div className="space-y-4 pt-4 border-t border-zinc-900">
                        <div className="rounded-xl bg-zinc-900/50 border border-zinc-800/80 p-5 backdrop-blur-sm">
                            ...
                            <button className="mt-4 w-full rounded-md bg-zinc-800 hover:bg-zinc-700 py-2 text-xs font-medium text-zinc-300 transition-colors border border-zinc-700/50">
                                Buy Credits
                            </button>
                        </div>
                        ...
                        <div>
                            <button
                                ...
                                className={`w-full flex items-center justify-between p-3.5 rounded-xl border text-xs font-medium transition-all duration-300 ${workspace?.status === 'paused'
                                    ...
                                    : 'bg-zinc-900/30 border-zinc-800/50 hover:border-zinc-700 hover:bg-zinc-800/50'
                                    }`}
                            >
                                <div className="flex flex-col items-start gap-1">
                                    <span className={workspace?.status === 'paused' ? 'text-amber-400 font-semibold' : 'text-zinc-400'}>
                ...
                <div className="border-t border-zinc-800/50 p-4">
                    <div className="flex items-center px-3">
                        <div className="h-8 w-8 rounded-full bg-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold text-xs ring-1 ring-indigo-500/30">
                            {workspace?.brandName?.[0] || 'A'}
                        </div>
                        <div className="ml-3">
                            <p className="text-sm font-medium text-white">{workspace?.brandName || 'Agency'}</p>
                ...
                <div className="border-t border-zinc-800/50 p-4 flex items-center justify-between">
                    <button onClick={toggleTheme} className="p-2 rounded-full hover:bg-zinc-800 text-zinc-400 hover:text-white transition-colors">
